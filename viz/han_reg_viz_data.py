...
def evaluation(_compute_device, _model, _eval_data):
    with torch.no_grad():
        ...
        for step, inputs in enumerate(_eval_data):
            ...
            batch_seg, sent_order, num_asp = [
                v.to(_compute_device) for v in [batch_seg, sent_order, num_asp]
            ]
            pol_logits, asp_prob, var, [edu_att_score, word_att_score] = _model(
                batch_seg, num_word_seg, num_seg, sent_order, num_asp
            )
...
def evaluation_query(_compute_device, _model, _eval_data):
    with torch.no_grad():
        lst_all_inst = []
        lst_y_pred, lst_y_true = [], []
        for step, inputs in enumerate(_eval_data):
            ...
            for i in range(batch_size):
                ...
                asp_labels = [
                    [*ASP_DICT][idx]
                    for idx, v in enumerate(y_asp_ture[i].tolist())
                    if v == 1
                ]
                sem_labels = [
                    polarity_labels[v]
                    for idx, v in enumerate(y_sem_true[i].tolist())
                    if v != -1
                ]
                labels = ["{}:{}".format(a, b) for a, b in zip(asp_labels, sem_labels)]
                ...
                inst = {
                    ...
                    "labels": labels,
                    "asp_prob": asp_prob,
                    "y_asp_true": y_asp_ture,
                    "y_sem_pred": y_sem_pred,
                    "asp_true": y_asp_ture[i].tolist(),
                    "sem_true": y_sem_true[i].tolist(),
                }
                lst_all_inst.append(inst)
        ...
        avg_f1 = f1_score(y_true=lst_y_true, y_pred=lst_y_pred, average="macro")
        f1s = f1_score(
            y_true=lst_y_true, y_pred=lst_y_pred, average=None, labels=[1, 0, 2]
        )
        acc = accuracy_score(y_true=lst_y_true, y_pred=lst_y_pred)
        cfm = confusion_matrix(y_true=lst_y_true, y_pred=lst_y_pred, labels=[1, 0, 2])
        print(avg_f1, acc)
...
def generate_segs2idx(dataset, vocab):
    query_dicts = dataset["query"]
    for d in query_dicts:
        if "segs" in d:
            ...
            for s in segs:
                words = s.lower().split(" ")
                idx = [vocab[word] for word in words if word in vocab]
                segs2idx.append(idx)
...
def get_indices_above_threshold(tensor, threshold=0.5):
    ...
    # Extract the column indices from the tensor and convert them to a list
    column_indices = indices[:, 1].tolist()

    return column_indices


def interpret_attention_scores(query_lst_all_inst, ASP_DICT, threshold=0.5):
    query = copy.deepcopy(query_lst_all_inst)
    
    for inst in query:
        signif_asp_idx = get_indices_above_threshold(inst["asp_prob"], threshold)
        for idx in signif_asp_idx:
            inst["asp_true"][idx] = 1.0
            inst["sem_true"][idx] = inst["y_sem_pred"][0, idx]
            inst["labels"].append(
                "{}:{}".format(list(ASP_DICT.keys())[idx], polarity_labels[inst["y_sem_pred"][0, idx]])
            )
        rem_keys = ["asp_prob", "y_asp_true", "y_sem_pred"]
        for k in rem_keys:
            inst.pop(k, None)
...
def store_attention_scores_from_input(input):
    ...
    np.random.seed(seed=default_random_state)
    torch.manual_seed(default_random_state)
    torch.cuda.manual_seed(default_random_state)
    ...
    # dataset = json.load(open('exp_data/sample.json', 'r'))
    if type(input) != dict:
    ...
    voacb_vocab = vocab_embed["vocab"]
    dataset = generate_segs2idx(dataset, voacb_vocab)
    glove = vocab_embed["glove"]
    glove_asp = [
        vocab_embed["glove"][vocab_embed["vocab"][k]] for k, v in ASP_DICT.items()
    ]
    glove_asp = np.array(glove_asp)
    aspect_indexes = [vocab_embed["vocab"][k] for k, v in ASP_DICT.items()]
    print(len(dataset["query"]))
    ...
    model = load_model(_model_name="han_reg", _data_name="rest", _model=model)
    model.eval()

    query_avg_f1, query_f1s, query_acc, query_cfm, query_lst_all_inst = evaluation_query(
        compute_device, model, query_dataloader.Data
    )
    # pdb.set_trace()
    # pickle.dump({'hard': hard_lst_all_inst, 'test': test_lst_all_inst}, open('sample.raw', 'wb'))
    query_lst_all_inst_parsed = interpret_attention_scores(
        query_lst_all_inst=query_lst_all_inst, ASP_DICT=ASP_DICT, threshold=0.5
    )
...
def store_attention_scores_sample():
    parser = argparse.ArgumentParser()
    parser.add_argument("--const_random_sate", type=int, default=42)
    FLAGS = parser.parse_args()
    print(FLAGS)
...
def store_attention_scores():
    ...
    test_dataloader = DatasetLoader(
        batch_size=512, asp_dict=ASP_DICT, input_data=dataset["test"]
    )
    hard_dataloader = DatasetLoader(
        batch_size=512, asp_dict=ASP_DICT, input_data=dataset["hard"]
    )
    del dataset
    ...
    test_avg_f1, test_f1s, test_acc, test_cfm, test_lst_all_inst = evaluation(
        compute_device, model, test_dataloader.Data
    )
    hard_avg_f1, hard_f1s, hard_acc, hard_cfm, hard_lst_all_inst = evaluation(
        compute_device, model, hard_dataloader.Data
    )
    # pdb.set_trace()
    pickle.dump(
        {"hard": hard_lst_all_inst, "test": test_lst_all_inst}, open("test1.raw", "wb")
    )
    print("score finished.")


def viz_attention():
    rest_han = pickle.load(open("model_files/sample.raw", "rb"))
    hard = rest_han["query"]
    # test = rest_han['test']
    for idx, inst in enumerate(hard):
        ...
        asp_true_idx = {
            a_i: [*ASP_DICT][a_i] for a_i, a_v in enumerate(asp_true) if a_v == 1.0
        }
        print("=" * 100)
        print("|".join(segs))
        print(labels)
        ...
        for seg_idx, seg in enumerate(segs):
            ...
            for a_idx, asp in asp_true_idx.items():
                asp_edu_score = "{}({:.2f})".format(
                    asp, edu_score[a_idx, seg_idx] * 100
                )

                asp_seg_word_scores = word_score[a_idx, seg_idx].tolist()
                asp_seg_score_text = []
                for t_idx, term in enumerate(seg_terms):
                    asp_seg_score_text.append(
                        "{}({:.2f})".format(term, asp_seg_word_scores[t_idx] * 100)
                    )

                asp_seg_text = "{} --> {}".format(
                    asp_edu_score, " ".join(asp_seg_score_text)
                )
                print(asp_seg_text)
            print("-" * 20)
...
def attention_accuracy():
    ...
    for idx, inst in enumerate(test):
        ...
        asp_true = inst["asp_true"]
        sem_true = inst["sem_true"]
        if len(segs) == 1 or sum(asp_true) == 1:
            continue
        cnt += 1
        """
